...
- [ ] **Step 1: Write the failing prompt and parser tests.**
...
- [ ] **Step 4: Run the focused package tests and commit the contract slice.**
...
- [ ] **Step 1: Write failing manager, store, and endpoint tests.**
...
- [ ] **Step 2: Run the focused tests and confirm they fail for missing fields and route.**
...
- [ ] **Step 3: Add model, migration, and manager transitions.**
...
- [ ] **Step 4: Add the protected Done endpoint and event publication.**
...
- `AgentsView` consumes `onMarkDone(id): Promise<void> | void` in addition to the existing terminal selection callback.
...
- [ ] **Step 2: Run the focused Web tests and confirm the expected red state.**
...
- [ ] **Step 3: Implement the minimal view and API behavior.**
...
- [ ] **Step 4: Wire App Done reconciliation and action reopening.**
...
- [ ] **Step 5: Run focused tests, typecheck, and commit the Web slice.**
...
Intercept `/api/sessions` and `/api/agent-summaries` with one waiting high-priority unread summary and one running summary, open Agents, assert the priority badge and bold-state data attribute, press the checkmark, assert the Done tab and Done card, then emit an updated summary with a changed action and `done:false` and assert it returns to Action required. Do not invoke a real provider.
...
- [ ] **Step 3: Run React Doctor and inspect the final diff.**
...
- [ ] **Step 4: Commit documentation updates and merge automatically.**
